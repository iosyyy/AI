...
class App extends Component {
    ...
    componentDidMount() {
        ...
        let socket = new WebSocket(localStorage.getItem("role") !== "guest" ? api.hostStatus : api.guestStatus);
                ...
                {this.state.count !== 0 ?this.state.datasource.map((item,index) => {
                    ...
                    return (
                        <Card key={index} hoverable style={{margin: "1vh"}} size='small'>
                            <Row>{role ? `请上传service_id为${item.f_service_id}对应的预测文件` : `参与方已上传关于service_id为${item.f_service_id}的对应预测文件`}</Row>
